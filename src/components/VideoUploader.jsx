  ...
  const handleStartProcess = async () => {
    ...
    try {
      ...
      const sessionRecord = await saveVideoSession({
        ...
        title: videoTitle.trim() || selectedFile.name.replace(/\.[^/.]+$/, "").replace(/_/g, " "),
      ...
      appendLog('Saved to local storage pool! Content accumulated locally. Open Cloud Sync to publish all master content.');
      setDrivePublishStatus('Saved to local storage pool. Ready for master cloud publish in Cloud Sync.');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className="p-4 rounded-2xl bg-[var(--bg-surface)] border border-[var(--border-color)] space-y-3">
            {/* Video Lecture Title Field */}
            <div>
              <label className="flex items-center space-x-1.5 text-xs font-semibold text-[var(--text-primary)] mb-1 font-heading">
                <Sparkles className="w-3.5 h-3.5 text-[var(--accent-coral)]" />
                <span>Video Lecture Title</span>
              </label>
              <input
                type="text"
                placeholder="e.g. Lecture 1: Introduction to Quantum Mechanics"
                value={videoTitle}
                onChange={(e) => setVideoTitle(e.target.value)}
                className="w-full bg-[var(--bg-ground)] border border-[var(--border-color)] rounded-xl px-3.5 py-2 text-xs text-[var(--text-primary)] font-heading placeholder-[var(--text-muted)] focus:outline-none focus:border-[var(--accent-coral)] transition"
              />
            </div>
